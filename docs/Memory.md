...
* **Active-Low Enable ($\overline{\text{Enable}}$):** Marked by the inversion bubble on the top control line. Driven low ($0\text{ V}$) to assert the nibble onto the bus, and pulled high ($5\text{ V}$) to disconnect.
